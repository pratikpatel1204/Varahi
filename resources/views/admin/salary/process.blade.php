...
@section('content')
                        ...
                        <select class="form-select" id="salaryYear">
                            ...
                            @foreach ($years as $year)
                                <option value="{{ $year->year }}"
                                    {{ $selectedYear == $year->year || (!$selectedYear && $year->status == 'Active') ? 'selected' : '' }}>
                                    {{ $year->year }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-select" id="salaryMonth">
                            ...
                            @foreach ($months as $month)
                                <option value="{{ $month->month_name }}"
                                    {{ $selectedMonth == $month->month_name || (!$selectedMonth && $month->status == 1) ? 'selected' : '' }}>
                                    {{ $month->month_name }}
                                </option>
                            @endforeach
                        </select>
    ...
    <script>
        ...
        function fetchProcessStatus() {
            ...
            fetch("{{ route('admin.salary.process.status') }}?year=" + year + "&month=" + month)
                .then(res => res.json())
        ...
        $(document).on('click', '.verify-attendance', function() {
            redirectTo("{{ route('admin.salary.attendance.preview') }}");
        });

        $(document).on('click', '[data-step="2"] button', function() {
            redirectTo("{{ route('admin.salary.loan.preview') }}");
        });

        $(document).on('click', '[data-step="3"] button', function() {
            redirectTo("{{ route('admin.salary.expense.preview') }}");
        });

        $(document).on('click', '[data-step="4"] button', function() {
            redirectTo("{{ route('admin.salary.process.verify') }}");
        });

        $(document).on('click', '[data-step="5"] button', function() {
            redirectTo("{{ route('admin.salary.slip.page') }}");
        });
    </script>
